...
/**
 * 线程池中的线程分为：核心线程和非核心线程。当线程池中的线程小于核心线程数的数量的时候，
 * 会新建线程直到达到核心线程数量。
 *
 * 其中用户向线程池中submit的任务，会先进入一个线程安全的队列中，被线程池中的线程消费
 *
 * 当线程池中的线程数量等于核心线程数，并且队列满了时，会新创建线程数直到非核心线程数量。
 * 这样以后队列还是满了那么没有入队的任务将会被丢弃
 * (常用的策略有:丢弃当前任务、丢弃最老的任务、调用者自己处理、抛出异常等)
 *
 */
    ...
    private boolean addWorker(Runnable newTask, boolean core) {
        ...
        for(;;){
            ...
            if(runningCount.compareAndSet(count, count+1)){
                // 线程的名字
                String thread_name = (core ? "core_" : "") + name + sequence.incrementAndGet();
                // 创建线程并启动任务
                new Thread(()->{
